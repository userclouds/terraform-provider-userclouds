...
var _ resource.Resource = &UserstoreColumnResource{}
var _ resource.ResourceWithImportState = &UserstoreColumnResource{}

// NewUserstoreColumnResource returns a new instance of the resource.
func NewUserstoreColumnResource() resource.Resource {
	return &UserstoreColumnResource{}
}
...
func (r *UserstoreColumnResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_userstore_column"
}
...
func (r *UserstoreColumnResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: "Manages a User Store column. For more details, refer to the [User Store documentation](https://docs.userclouds.com/docs).",
		Attributes:          UserstoreColumnAttributes,
	}
}

// Configure configures the resource.
func (r *UserstoreColumnResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *UserstoreColumnResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var data *UserstoreColumnTFModel
	...
	jsonclientModel, err := UserstoreColumnTFModelToJSONClient(data)
	if err != nil {
		resp.Diagnostics.AddError("Error converting userclouds_userstore_column to JSON", err.Error())
		return
	}

	url := "/userstore/config/columns"
	...
	body := IdpCreateColumnRequestJSONClientModel{
		Column: jsonclientModel,
	}

	marshaled, err := json.Marshal(body)
	if err != nil {
		resp.Diagnostics.AddError("Error serializing userclouds_userstore_column JSON request body", err.Error())
		return
	}
	...
	tflog.Trace(ctx, "successfully created userclouds_userstore_column with ID "+created.ID.String())
	resp.Diagnostics.Append(resp.State.Set(ctx, &createdTF)...)
}

// Read reads the existing resource state.
func (r *UserstoreColumnResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	var oldState *UserstoreColumnTFModel
	...
	tflog.Trace(ctx, "successfully read userclouds_userstore_column with ID "+current.ID.String())
	resp.Diagnostics.Append(resp.State.Set(ctx, &newState)...)
}

// Update updates an existing resource.
func (r *UserstoreColumnResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var state *UserstoreColumnTFModel
	resp.Diagnostics.Append(req.State.Get(ctx, &state)...)

	var plan *UserstoreColumnTFModel
	...
	jsonclientModel, err := UserstoreColumnTFModelToJSONClient(plan)
	if err != nil {
		resp.Diagnostics.AddError("Error converting userclouds_userstore_column to JSON", err.Error())
		return
	}
	...
	tflog.Trace(ctx, "successfully updated userclouds_userstore_column with ID "+updated.ID.String())
	resp.Diagnostics.Append(resp.State.Set(ctx, &newState)...)
}

// Delete deletes an existing resource.
func (r *UserstoreColumnResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data *UserstoreColumnTFModel
	...
	if err := r.client.Delete(ctx, url, nil); err != nil {
		resp.Diagnostics.AddError("Error deleting userclouds_userstore_column", err.Error())
		return
	}
}

// ImportState imports an existing resource into Terraform.
func (r *UserstoreColumnResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
